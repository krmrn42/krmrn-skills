...
- **Machine-launched JSONL filtering** (spec §D15) — three classes of session no longer manifest as phantom projects: (a) subagent transcripts under `<conv-id>/subagents/`, (b) Claude Agent SDK launches (`entrypoint: "sdk-cli"` — e.g., skill-creator scaffolding, scripted `/tmp/probe-*` invocations), (c) sidechain sessions (`isSidechain: true`). Each JSONL is locked to its session-start `cwd` (fixes per-message `cwd` drift from controller-side `cd` calls); rows from all three classes are flagged `is_subagent = 1` and filtered out of `searchProjects` / `recentConversations` / `ftsSearch`. The rows remain in the index for future opt-in surfacing. Schema bumps v3 → v4 with the new `is_subagent` column, triggering a one-time drop-rebuild on first run after upgrade.
