...
Search the user's past Claude Code conversations using the `ccsearch` engine bundled with this plugin, and render the ranked results inline.

The plugin's `bin/` directory is automatically added to the `Bash` tool's `PATH` while the plugin is enabled, so `ccsearch` is callable by bare name. Invoke it via the `Bash` tool with the user's arguments and the fixed flags `--format=text --no-color --limit=10`:

```bash
ccsearch $ARGUMENTS --format=text --no-color --limit=10
```

`$ARGUMENTS` is forwarded verbatim, so any of `ccsearch`'s flags (`--project`, `--since`, `--regex`, `--include-tools`, etc.) can be passed by the user inline:
...
If — and only if — the bare-name invocation returns `ccsearch: command not found` (a sign the plugin's `bin/` PATH integration is broken), fall back to `${CLAUDE_PLUGIN_ROOT}/bin/ccsearch` with the same arguments AND surface a one-line note to the user explaining the PATH integration appears broken so they can file an issue. Do **not** try `${CLAUDE_PLUGIN_ROOT}/bin/ccsearch` first as a precaution — the bare name is the documented path.

## How to render the result

`ccsearch` text output is already formatted with a per-result resume one-liner of the form `(cd <project_path> && claude --resume <session_id>)`. Show the output verbatim — that one-liner per result is the copy-paste handoff to the user.
...
- **Exit 2:** environment problem. The most common cases are:
  ...
  - Schema drift after a plugin upgrade — surface verbatim and recommend `ccsearch --reindex` to rebuild the index.
- **Exit 3:** internal error. Surface verbatim and recommend filing an issue against the plugin.

## When to point at `ccsearch -i` instead
...
> "If you want to scroll through more candidates with a live preview pane, run `! ccsearch -i $ARGUMENTS` from the prompt — that opens the built-in TUI picker in your real terminal where you can pick a chat with Enter (which lands you in the right project directory automatically)."

Do **not** try to invoke the picker from this slash command, and do **not** mention `fzf` — the picker has no such dependency.

If the user asks how to make `ccsearch` callable from their own shell (outside Claude Code), point them at `/chat-search:setup` — it's a one-shot helper that symlinks `bin/ccsearch` into `~/.local/bin` and tells them whether their PATH needs an export line.
