...
- **[The OpenSpec capability folder name `ccsearch-cli-help` was misleading]** — After this change, the CLI is `multivac` but the capability folder under `openspec/specs/` initially still said `ccsearch-cli-help`. → **Resolution:** the capability was renamed `ccsearch-cli-help` → `multivac-cli-help` (along with the other `ccsearch-*` capabilities) in a follow-up cleanup; the spec text describes the multivac CLI throughout. References to the archived change `improve-ccsearch-help` keep their original name.
