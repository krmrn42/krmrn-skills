# multivac-default-mode Specification
...
When invoked on a terminal where both stdin and stdout are TTYs, and no opt-out flag is present (see next requirement), `multivac` SHALL open the built-in TUI picker. This holds with or without a positional query: a bare `multivac` MUST open the picker with an empty query (no longer the historic "no query provided" error), and `multivac "foo"` MUST open the picker pre-populated with `foo`.

#### Scenario: Bare invocation on TTY opens picker

- **WHEN** the user runs `multivac` with stdin and stdout both attached to a terminal, no flags
- **THEN** the TUI picker opens with an empty query field
- **AND** the picker behaves identically to today's `multivac -i` (no query)
- **AND** the process does NOT emit the previous `"no query provided"` error

#### Scenario: Query-only invocation on TTY opens picker

- **WHEN** the user runs `multivac "session timeout"` on an interactive TTY
...
The following flags, when present, SHALL cause `multivac` to dispatch to one-shot rendering (the pre-change default) even on an interactive TTY: `--list` / `-l`, `--format=text`, `--format=tsv`, `--regex` (with or without `--scan`), `--preview SESSION_ID`, `--reindex`, `--index-status`.

In addition, when stdin or stdout is NOT a TTY, `multivac` MUST dispatch to one-shot rendering regardless of whether any opt-out flag is present.

#### Scenario: --list forces one-shot on a TTY

- **WHEN** the user runs `multivac --list "foo"` on an interactive TTY
- **THEN** the process prints the same text output that `multivac "foo"` produced before this change
- **AND** the TUI does NOT open

#### Scenario: --format=text forces one-shot on a TTY

- **WHEN** the user runs `multivac --format=text "foo"` on an interactive TTY
...
- **WHEN** the user runs `multivac "foo" > out.txt`
- **THEN** the process renders `tsv` and exits cleanly
- **AND** the TUI does NOT open

#### Scenario: Non-TTY stdin forces one-shot

- **WHEN** the user runs `echo "" | multivac "foo"` (stdin is a pipe even though stdout is a TTY)
...
The `-i` and `--interactive` flags SHALL continue to be accepted by the parser. When present they MUST select the TUI picker, identical to today's behavior. They take precedence over opt-out flag inference, EXCEPT they cannot override a non-TTY environment: `multivac -i` with non-TTY stdin or stdout MUST fail with the existing `EXIT_ENV` ("requires a TTY") error rather than silently falling back to one-shot.

#### Scenario: -i still opens the picker on a TTY

- **WHEN** the user runs `multivac -i "foo"` on an interactive TTY
- **THEN** the TUI picker opens with `foo` pre-filled, exactly as before this change

#### Scenario: -i with --list errors clearly

- **WHEN** the user runs `multivac -i --list "foo"`
...
- **WHEN** the user runs `multivac -i "foo" | cat` (stdout piped)
...
The output of `multivac --help` SHALL reflect that the TUI is the default and that `--list`/`--format`/`--regex`/special-modes opt out of it. Specifically: the `-i` / `--interactive` entry MUST note that it is the default on a TTY; a `--list` / `-l` entry MUST be present and describe it as "force one-shot ranked text output (the pre-change default)"; the usage synopsis MUST show that the positional `[query]` is optional. (Coordinated with the `improve-multivac-help` change, which lands the structured help format these entries live in.)
...
The slash command `/chat-search:find` invokes `multivac --format=text --no-color --limit=10`. Because `--format` is set, this falls into the one-shot opt-out branch by the rules above. The slash command's documented behavior MUST be unchanged after this change.

#### Scenario: /chat-search:find still renders inline text

- **WHEN** the slash command runs in a Claude Code session and invokes `multivac --format=text --no-color --limit=10 "foo"`
