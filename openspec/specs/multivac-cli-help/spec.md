...
- **WHEN** a user runs `multivac --help`
- **THEN** the output describes the meaning of exit codes `0`, `1`, `2`, and `3` (matching the values defined at the top of `bin/multivac`)
...
`multivac -h` and `multivac --help` SHALL each write the help text to stdout (not stderr) and exit with status `0`, regardless of whether other flags or a positional query are present on the command line.
...
- **WHEN** a user runs `multivac -h`
- **THEN** the output and exit status match `multivac --help` byte-for-byte

#### Scenario: --help overrides other flags

- **WHEN** a user runs `multivac --help --regex 'foo' --limit 5`
...
The `multivac` test suite SHALL include a check that fails if the help text omits any long-form flag accepted by the parser, so future flag additions cannot be merged without updating the help.
...
- **THEN** running `bash plugins/chat-search/bin/multivac.test.sh` exits non-zero with a message naming the flag(s) missing from help
